function prepare_QRdata(content) {
  ...
  let hash_code;
    ...
    hash_code = myObj.hash_code + myObj.security_digit;
    request_data(sender_id, shipnum, hash_code);
}
...
function prepare_data_man () {
    ...
    content = "(vacio)";
    request_data(sender_id, shipnum, content);
...
function request_data(sender_id, shipnum, content) {
    content = content || 'INGRESADO_A_MANO';
    mat_ship = [];
    var Ajax;
    ...
    Ajax.open("GET","./info_shipping.php?sender_id="+sender_id+"&shipnum="+shipnum+"&sticker="+content,true);
