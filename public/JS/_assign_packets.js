...
function pulsar() {
  ...
  if (cadete_valido == true) {
    document.getElementById("selec_definitivo").value = document.getElementById("input_dummy").value.slice(-6);
    document.getElementById("boton_definitivo").click();
  } else {
...
function submit_array() {
    // Esta parte del código evalua que el cadete sometido en el datalist está en la lista inicial (OJO es sólo para el escaner, para la ista la subrutina se llama pulsal)
    for (i = 0; i < document.getElementById("Cadetexy").children.length; i++) {
       if (document.getElementById("cadete_dummy").value == document.getElementById("Cadetexy").children[i].value) {
         cadete_valido = true;
       }
    ...
    // Si el cadete es válido continúa la ejecución en caso contrario lanza la alerta
    if (cadete_valido == true) {
        ...
        if (document.getElementById('cadete_dummy').value != "") { 
            ...
            document.getElementById('cadete_scann').value = document.getElementById('cadete_dummy').value.slice(-6);
